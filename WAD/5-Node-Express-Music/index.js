...
const db_connection =
  "mongodb+srv://mitali:mitali2004@music.7qyok8a.mongodb.net/?retryWrites=true&w=majority&appName=music";
const port = 2000;
...
app.get(
  "/getSongsOfDirector/:directorName",
...
app.get(
  "/getSongsOfDirectorAndSinger/:directorName/:singerName",
  async function (request, response) {
    const directorName = request.params.directorName;
    const singerName = request.params.singerName;
    const songs = await Songdetails.find({
      Music_director: directorName,
      singer: singerName,
    });
    response.send(songs);
  }
...
app.get(
  "/getSongsOfSingerAndFilm/:singerName/:filmName",
  async function (request, response) {
    const singerName = request.params.singerName;
...
app.put("/updateActorAndActress", async function (request, response) {
  ...
  const song = await Songdetails.findOneAndUpdate(
    ...
    { new: true }
  ); // {new:true} is used for getting latest updated data
  response.send(song);
});
...
console.log(
  "waiting for database to connect. after connection server will start..."
);
mongoose
